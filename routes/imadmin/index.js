...
var utils = require('../../lib/utils')
  ...
  , Defauts = {
      defaultProject: 'Default Project'  // 在用户未指定默认项目时，系统默认的默认项目
  }
    ...
    generateUser: function (req, res, next) {
        ...
        db.model('user', schemas.user).findById(ur._id, function (err, user) {
            if (!err) {
                ...
                user.defaultProject = defaultProject || { projName: defaultProject };
                req.renderUser = user;
                next();
            };
            db.close();
    ...
    useDataBase: function (req, res, next) {
        ...
        if (ur) {
            ...
            dbString = [
                            ...
                            utils.md5([
                                        ur.userCert, '_',
                                        defaultProject ? defaultProject.projCert : utils.md5(Defauts.defaultProject)
                                       ].join(''))
                        ].join('');
